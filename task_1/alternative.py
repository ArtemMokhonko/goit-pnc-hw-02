...
def enhanced_terminal_output(encrypted_text, key_length, found_key, segments, decrypted):
    """Покращений вивід в термінал із поясненнями"""
    print("\n" + "="*50)
    print("Кроки криптоаналізу шифру Віженера")
    print("="*50)
    print("\nМетод Касіскі:")
    ...
    print(f"\n1. Визначення довжини ключа (ключ має довжину {key_length}).")
    print("\nРезультати методу Касіскі та тесту Фрідмана вказують на цю довжину.")
    print(f"\n2. Знайдений ключ: {found_key}.")
    print("\n3. Сегменти тексту, що використовувались для аналізу:\n")
    for i, segment in enumerate(segments):
        print(f"Сегмент {i+1}: {segment}")
    print("\n4. Результати дешифрування:")
    print(decrypted)
...
if __name__ == "__main__":
    ...
    decrypted_text = vigenere_decrypt(encrypted_text, found_key)
    
    # Отримання сегментів
    clean_text = get_clean_text(encrypted_text)
    segments = [''.join(clean_text[i::key_length]) for i in range(key_length)]
    
    # Покращений вивід
    enhanced_terminal_output(encrypted_text, key_length, found_key, segments, decrypted_text)
