...
def create_matrix(text: str, num_cols: int, fill: str = ' ') -> List[List[str]]:
    """
    Створює матрицю тексту за кількістю колонок.

    :param text: Вхідний текст для обробки.
    :param num_cols: Кількість колонок (довжина ключа).
...
def transposition_col_encrypt(text: str, key: str, log: bool = False) -> str:
    """
    Шифрує текст за методом перестановки колонок.
    ...
    # Визначаємо порядок колонок на основі ключа
...
def transposition_col_decrypt(encrypted_text: str, key: str, log: bool = False) -> str:
    """
    Розшифровує текст за методом перестановки колонок.
...
def double_transposition_encrypt(text: str, key_row: str, key_col: str, log: bool = False) -> str:
    """
    Шифрує текст за методом подвійної перестановки.

    :param text: Вхідний текст для шифрування.
    :param key_row: Ключ для перестановки рядків.
    :param key_col: Ключ для перестановки колонок.
...
def double_transposition_decrypt(encrypted_text: str, key_row: str, key_col: str, log: bool = False) -> str:
    """
    ...
    # Відновлення матриці з перестановкою колонок
    col_indices = sorted(range(num_cols), key=lambda k: key_col[k])
    matrix = [[''] * num_cols for _ in range(num_rows)]
    index = 0
    ...
    if log:
        print("Матриця після перестановки колонок (ключ: {}):".format(key_col))
        for row in matrix:
            print(row)
...
if __name__ == "__main__":
    ...
    # Шифрування тексту
    encrypted = transposition_col_encrypt(original_text, key)
    print(f"Зашифрований текст методом перестановки колонок:\n{encrypted}\n")

    # Дешифрування тексту
    decrypted = transposition_col_decrypt(encrypted, key)
    print(f"Дешифрований текст методом перестановки колонок:\n{decrypted}\n")
    ...
    # Шифрування тексту
    encrypted = double_transposition_encrypt(original_text, key_row, key_col, log=False)
    print(f"Зашифрований текст методом подвійної перестановки:\n{encrypted}\n")

    # Дешифрування тексту
    decrypted = double_transposition_decrypt(encrypted, key_row, key_col, log=False)
